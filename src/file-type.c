...
FileRet file_parse(const char* filename, FileTypeDetail* detail)
{
    ...
    do {
        ...
        ret = file_parse_header(fileHeader, fileSize, fileName, detail);
    } while (false);
...
FileRet file_parse_header(const char* fileHeader, int64_t fileSize, const char *fileName, FileTypeDetail *detail)
{
    C_RETURN_VAL_IF_FAIL(fileHeader && detail, RET_ERROR);

    if (fileName && c_str_has_suffix(fileName, ".java") && check_is_java_source(fileHeader)) {
